  ...
  module Services
    class SubscriptionService
      ...
      def pause_subscription(customer, coffee)
        subscription = find_subscriptions_by_customer(customer, 'active').find do |subscription|
      ...
      def cancel_subscription(customer, coffee)
        subscription = find_subscriptions_by_customer(customer).find do |subscription|
          subscription.coffee_name == coffee.name
        end
      ...
      def find_subscriptions_by_customer(customer, status = 'any')
        if status == 'active'
          ...
          @data_store.find_all_subscriptions_by_customer(customer)
        end
      end
    end
  end
